...
class BaseSampler(metaclass=ABCMeta):
    ...
    def sample(self, assign_result: AssignResult, pred_instances: InstanceData,
        ...
        if self.add_gt_as_proposals and len(gt_bboxes) > 0:
            ...
            print("="*30)
            print(f"type(gt_bboxes_): {type(gt_bboxes)}")
            print(f"type(priors): {type(priors)}")
            try:
                print(f"gt_bboxes_ device: {gt_bboxes.device}")
            except:
                print(f"gt_bboxes_.tensor.device: {gt_bboxes.tensor.device}")
            try:
                print(f"priors device: {priors.device}")
            except:
                print(f"priors.tensor.device: {priors.tensor.device}")
            print("="*30)
